// Modbus RTU 协议面板（协议注册表中的一项）。参考 MODBUS 调试助手（V1.0）：
// 01/02/03/04 读，05/06/0x10 写，任意指令 HEX/CRC 发送；响应在 onRaw 原始流里匹配消费。
// 传输复用当前标签页顶部栏连接（发送走 api.send），不做二次串口管理。
// 纯前端逻辑（app/src/modbus.ts），浏览器演示模式与桌面端行为一致。
//
// 作为 ProtoController 实现：onRaw/setConnected/snapshot/applySnapshot 由协议页统一调度。
...
import type { ProtoController, ProtoCtx } from "./protocol";
...
export class ModbusPanel implements ProtoController {
  ...
  constructor(host: HTMLElement, ctx: ProtoCtx) {
    this.host = host;
    this.api = ctx.api;
    this.getConnected = ctx.getConnected;
    this.build();
  }

  private q<T extends HTMLElement>(sel: string): T | null {
    return this.host.querySelector<T>(sel);
  }
  ...
  private wireArbitrary(): void {
    const card = this.q<HTMLElement>("[data-arb]")!.closest(".proto-card") as HTMLElement;
  ...
  setConnected(on: boolean): void {
    this.host.querySelectorAll<HTMLButtonElement>(".proto-go").forEach((b) => (b.disabled = !on));
  }

  snapshot(): Record<string, string> {
    const store: Record<string, string> = {};
    let i = 0;
    for (const input of this.host.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>(".proto-in")) {
  ...
  applySnapshot(s: Record<string, string>): void {
    const inputs = this.host.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>(".proto-in");
    inputs.forEach((el, i) => {
      const v = s[`i${i}`];
